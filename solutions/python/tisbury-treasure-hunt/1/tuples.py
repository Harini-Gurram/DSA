...
def convert_coordinate(coordinate):
    ...
    """
    coordinate_converted=tuple(coordinate)
    return coordinate_converted
...
def compare_records(azara_record, rui_record):
    ...
    """
    azara_coordinates=tuple(azara_record[-1])
...
def clean_up(combined_record_group):
    ...
    for record in combined_record_group:
        azara_coordinate=tuple(record[1])
        if azara_coordinate==record[3]:
            tuple_string="('{0}', '{1}', {2}, '{3}')".format(record[0],record[2],record[3],record[4])
            cleaned_up_string+=tuple_string+"\n"
